...
def get_current_ip():
    ...
    with urlopen('https://api.ipify.org').read().decode('utf8') as current_ip:
...
def update_dreamhost_a_record(key, record, new_ip):
    ...
    with urlopen(records_url).read().decode('utf8') as records:
        ...
        if not re.search(record_match, records):
            ...
            with urlopen(remove_url).read().decode('utf8') as result:
        ...
        with urlopen(add_url).read().decode('utf8') as result:
